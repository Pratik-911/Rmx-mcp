    ...
    initializeTools() {
        const toolDefinitions = [
            ...
            },
            {
                ...
                description: "Get persona profile using project and persona names (more user-friendly)",
                inputSchema: {
                    type: "object",
                    properties: {
                        project_name: {
    ...
    async callTool(toolName, args, client) {
        ...
        switch (toolName) {
            ...
            case 'find_persona_by_name':
                const projectId = await client.resolveProjectId(args.project_id);
                return await client.findPersonaByName(projectId, args.persona_name);
            
            case 'get_user_stories_by_name':
                const resolvedProjectId = await client.resolveProjectId(args.project_name);
                const resolvedPersonaId = await client.resolvePersonaId(resolvedProjectId, args.persona_name);
