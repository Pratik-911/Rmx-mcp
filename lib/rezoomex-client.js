...
export class RezoomexApiClient {
    ...
    async getAllProjects() {
        try {
            ...
            // If no projects in user info, we need to discover them dynamically
            // Try common project ID patterns or check for accessible projects
            const potentialProjects = [];
            
            // Since we don't have a direct endpoint, we'll need to implement
            // project discovery based on user permissions
            throw new Error('No projects endpoint available. User-specific project discovery not yet implemented. Contact admin to add your project IDs to the system.');
