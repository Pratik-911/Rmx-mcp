...
export class RezoomexApiClient {
    ...
    async getAllProjects() {
        try {
            // First, try the proper projects listing endpoint
            try {
                const response = await this.makeRequest('/v1/requirements/projects');
                ...
                // If response.data is an array of projects directly
                ...
                this.logger.warn('Failed to fetch projects from /v1/requirements/projects endpoint', { error: error.message });
            }
            
            // Fallback: Check user info for project references
            const userInfo = await this.getUserInfo();
            if (userInfo.data.projects) {
                return {
                    success: true,
                    projects: userInfo.data.projects,
                    total: userInfo.data.projects.length,
                    timestamp: new Date().toISOString()
                };
            }
            
            // If no projects found through proper endpoints
            throw new Error('No accessible projects found for this user. Please ensure you have proper permissions or contact support.');
    ...
    async listProjects(page = 1, pageSize = 20) {
        ...
        try {
            const endpoint = `/v1/requirements/projects?page=${page}&pageSize=${pageSize}`;
            const response = await this.makeRequest(endpoint);
            
            return {
                success: true,
                projects: response.data.projects || response.data || [],
                total: response.data.total || (response.data.projects ? response.data.projects.length : response.data.length) || 0,
    ...
    async searchProjects(query, page = 1, pageSize = 20) {
        ...
        try {
            ...
            try {
                const endpoint = `/v1/requirements/projects/search?q=${encodeURIComponent(query)}&page=${page}&pageSize=${pageSize}`;
                const response = await this.makeRequest(endpoint);
                ...
                this.logger.warn('Search endpoint not available, falling back to client-side filtering', { error: error.message });
